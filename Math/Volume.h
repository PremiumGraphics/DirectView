...
public:
	...
	//  Orignal source code is developed by https://code.google.com/p/kvs/.
	std::vector< Math::Vector3d<float> > Volume::createTornadoVolume(const float time)
	{
		const double dx = 1.0f / (dimX - 1.0f);
		const double dy = 1.0f / (dimY - 1.0f);
		const double dz = 1.0f / (dimZ - 1.0f);

		std::vector< Math::Vector3d<float> > vectors;
			...
			for (int j = 0; j < dimY; j++) {
				...
				for (int i = 0; i < dimX; i++) {
					...
					*/
					...
					Math::Vector3d<float> vector(v[0], v[1], v[2]);
